...
int MPI_Send(const void *buf, int count, MPI_Datatype datatype,
    ...
    if (ret == MPI_SUCCESS) {
        ...
        record_metric("MPI_Send", dest, tag,
...
int MPI_Recv(void *buf, int count, MPI_Datatype datatype,
    ...
    if (ret == MPI_SUCCESS) {
        ...
        if (status != MPI_STATUS_IGNORE && status != NULL) {
            ...
            real_tag = status->MPI_TAG;
        }

        int type_size = 0;
        PMPI_Type_size(datatype, &type_size);

        record_metric("MPI_Recv", real_source, real_tag,
...
int MPI_Isend(const void *buf, int count, MPI_Datatype datatype,
    ...
    if (ret == MPI_SUCCESS) {
        ...
        record_metric("MPI_Isend", dest, tag,
                      (uint64_t)count * (uint64_t)type_size,
                      end - start);
    }
...
int MPI_Irecv(void *buf, int count, MPI_Datatype datatype,
    ...
    if (ret == MPI_SUCCESS) {
        ...
        record_metric("MPI_Irecv", source, tag,
                      (uint64_t)count * (uint64_t)type_size,
                      end - start);
    }
...
int MPI_Barrier(MPI_Comm comm) {
    ...
    if (ret == MPI_SUCCESS) {
        record_metric("MPI_Barrier", -1, -1, 0, end - start);
    }
...
int MPI_Bcast(void *buffer, int count, MPI_Datatype datatype,
    ...
    if (ret == MPI_SUCCESS) {
        ...
        record_metric("MPI_Bcast", root, -1,
                      (uint64_t)count * (uint64_t)type_size,
                      end - start);
    }
...
int MPI_Wait(MPI_Request *request, MPI_Status *status) {
    ...
    if (ret == MPI_SUCCESS) {
        ...
        record_metric("MPI_Wait", real_source, real_tag, 0, end - start);
    }
...
int MPI_Waitall(int count, MPI_Request array_of_requests[],
    ...
    if (ret == MPI_SUCCESS) {
        record_metric("MPI_Waitall", -1, -1, 0, end - start);
    }
...
int MPI_Reduce(const void *sendbuf, void *recvbuf, int count,
    ...
    if (ret == MPI_SUCCESS) {
        ...
        record_metric("MPI_Reduce", root, -1,
                      (uint64_t)count * (uint64_t)type_size,
                      end - start);
    }
...
int MPI_Allreduce(const void *sendbuf, void *recvbuf, int count,
    ...
    if (ret == MPI_SUCCESS) {
        ...
        record_metric("MPI_Allreduce", -1, -1,
                      (uint64_t)count * (uint64_t)type_size,
                      end - start);
    }
...
int MPI_Gather(const void *sendbuf, int sendcount, MPI_Datatype sendtype,
    ...
    if (ret == MPI_SUCCESS) {
        ...
        record_metric("MPI_Gather", root, -1,
...
int MPI_Scatter(const void *sendbuf, int sendcount, MPI_Datatype sendtype,
    ...
    if (ret == MPI_SUCCESS) {
        ...
        record_metric("MPI_Scatter", root, -1,
...
static metric_entry_t *get_entry(const char *operation, int peer, int tag) {
...
static void flush_metrics(void) {
    ...
    for (int i = 0; i < MAX_ENTRIES; i++) {
        ...
        fprintf(f,
                "mpi_calls_total{rank=\"%d\",operation=\"%s\",peer=\"%d\",tag=\"%d\"} %llu\n",
                rank_id,
                entries[i].operation,
                entries[i].peer,
                entries[i].tag,
        ...
        fprintf(f,
                "mpi_bytes_total{rank=\"%d\",operation=\"%s\",peer=\"%d\",tag=\"%d\"} %llu\n",
                rank_id,
                entries[i].operation,
                entries[i].peer,
                entries[i].tag,
        ...
        fprintf(f,
                "mpi_duration_seconds_total{rank=\"%d\",operation=\"%s\",peer=\"%d\",tag=\"%d\"} %.9f\n",
                rank_id,
                entries[i].operation,
                entries[i].peer,
                entries[i].tag,
...
static void record_metric(const char *operation, int peer, int tag,
    ...
    metric_entry_t *e = get_entry(operation, peer, tag);
